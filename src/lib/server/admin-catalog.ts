...
import { categories, products as starterProducts, type CategorySlug, type Product } from "@/data/catalog";
...
async function getDatabase() {
  ...
  const client = globals.__motoluxeCatalogMongoClient ?? new MongoClient(uri, { maxPoolSize: 10 });
...
export function validateCatalogInput(input: Record<string, unknown>) {
  ...
  const benefits = Array.isArray(input.benefits)
    ? input.benefits.filter((item): item is string => typeof item === "string").map((item) => item.trim()).filter(Boolean)
    : [];
  const usage = Array.isArray(input.usage)
    ? input.usage.filter((item): item is string => typeof item === "string").map((item) => item.trim()).filter(Boolean)
    : [];
  ...
  if (name.length < 2 || name.length > 120) return { error: "Enter a valid product name." as const };
  if (tagline.length < 2 || tagline.length > 200) return { error: "Enter a short product tagline." as const };
  if (!categories.some((item) => item.slug === category)) return { error: "Choose a valid category." as const };
  if (price.length < 1 || price.length > 80) return { error: "Enter a product price or pricing label." as const };
  if (size.length < 1 || size.length > 80) return { error: "Enter the product size or pack format." as const };
  if (image.length < 1 || image.length > 500) return { error: "Enter a product image path or URL." as const };
  if (description.length < 10 || description.length > 2000) return { error: "Enter a useful product description." as const };
  if (!Number.isInteger(stock) || stock < 0 || stock > 1_000_000) return { error: "Stock must be a whole number from 0 upward." as const };
...
export async function listCatalogProducts(search = "", category = "", stockFilter = "all") {
  ...
  if (category && categories.some((item) => item.slug === category)) query.category = category;
  if (search.trim()) {
